...
const SideBar = ({ toggle, setToggle }) => {
      ...
      return (<>
            <aside style={toggle ? asideToggleCss : {}} className="sidebar transition-all duration-500 bg-slate-950 h-full overflow-y-auto px-2 sm:static fixed right-[-350px] top-0 z-50 w-min(90%,350px) sm:w-auto ease-in-out">
                  <div className="font-syne h-20 flex items-center justify-center flex-col">
                        <div className="sm:hidden flex justify-start w-full px-5 h-full items-center">
                              <button onClick={() => setToggle(!toggle)} className='text-3xl text-slate-300'><i className="fa-solid fa-xmark"></i></button>
                        </div>
                        <a className="sm:block hidden" href='/'>
                              <h1 className="text-slate-100 text-4xl uppercase font-bold">Cine Search</h1>
                        </a>
                  </div>
                  ...
                  <div className="w-full min-h-[80svh] flex flex-col items-start justify-center gap-6 font-syne text-slate-300 p-5">
                        <div ><Link onClick={handleLinksClick} className='capitalize font-thin text-xl' to={`/`} >Home</Link></div>
                  ...
                  <div className="social-section p-5">
